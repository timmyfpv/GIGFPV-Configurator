...
export class Configure {
    ...
    static download = async (folder, version, deviceType, rxAsTxType, radioType, config, firmwareUrl, options) => {
        ...
        if (config.platform === "stm32") {
            const entry = await this.#fetch_file(firmwareUrl, 0, (bin) =>
            ...
            if (config.custom_layout) {
                ...
                const hardwareLayoutFile = await this.#fetch_file(
                    `${folder}/${version}/hardware/${deviceType}/${config.layout_file}`,
                    0,
            ...
            if (config.platform.startsWith("esp32")) {
                ...
                list.push(
                    this.#fetch_file(firmwareUrl, 0x10000, (bin) =>
                        Configure.#configureESP(deviceType, bin, config, options),
                    ),
                );
            } else if (config.platform === "esp8285") {
                list.push(
                    this.#fetch_file(firmwareUrl, 0x0, (bin) =>
                        Configure.#configureESP(deviceType, bin, config, options),
                    ),
                );
            }
            ...
            if (config.logo_file) {
                ...
                logoFile = await this.#fetch_file(`${folder}/${version}/hardware/logo/${config.logo_file}`, 0).catch(
                    () => this.#fetch_file(`${folder}/hardware/logo/${config.logo_file}`, 0),
                );
            }
