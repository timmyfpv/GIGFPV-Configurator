...
let Connector = class StlinkUsbConnector {
    ...
    async _write(data) {
        ...
        if (result.bytesWritten != data.length) {
            throw new Exception(
                `Error, only ${result.bytesWritten} Bytes was transmitted to ST-Link instead of expected ${data.length}`,
            );
        }
    }
...
const filters = DEV_TYPES.map((dev_type) => ({
