...
class Stm32 {
    ...
    async set_mem(addr, data) {
        ...
        while (true) {
            ...
            let write_size = Math.min(
                (data.length - written_size) & 0xfffffff8,
                this._stlink.maximum_transfer_size * 2,
            );
            if (write_size == 0) {
                break;
            }
    ...
    async flash_write(addr, data, { erase = false, verify = false, erase_sizes = null }) {
        ...
        this._dbg.debug(
            `Stm32.flash_write(${addr_str}, [data:${data.length}Bytes], erase=${erase}, verify=${verify}, erase_sizes=${erase_sizes})`,
        );
