...
export default class Stlink {
    ...
    async read_version() {
        ...
        if (this.ver_jtag < 21) {
            throw new Warning(
                `ST-Link/${this._ver_str} is not recent firmware, please upgrade first - functionality is not guaranteed.`,
            );
        }
    }
    ...
    async enter_debug_swd() {
        await this._connector.xfer([STLINK_DEBUG_COMMAND, STLINK_DEBUG_APIV2_ENTER, STLINK_DEBUG_ENTER_SWD], {
            rx_len: 2,
        });
    ...
    get_mem32(addr, size) {
        ...
        if (size > STLINK_MAXIMUM_TRANSFER_SIZE) {
            throw new Exception(
                `get_mem32: Size for reading is ${size} but maximum can be ${STLINK_MAXIMUM_TRANSFER_SIZE}`,
            );
        }
        let cmd = new ArrayBuffer(10);
        let view = new DataView(cmd);
    ...
    set_mem32(addr, data) {
        ...
        if (data.length > STLINK_MAXIMUM_TRANSFER_SIZE) {
            throw new Exception(
                `set_mem32: Size for writing is ${data.length} but maximum can be ${STLINK_MAXIMUM_TRANSFER_SIZE}`,
            );
        }
        let cmd = new ArrayBuffer(10);
        let view = new DataView(cmd);
