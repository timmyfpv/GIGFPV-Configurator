...
const FLASH_WRITER_F0_CODE = new Uint8Array([
    ...
    0x33,
    0x42, // 0x4233    // tst r3, r6
    0x04,
    0xd0, // 0xd104    // beq <exit>
    0x02,
...
class Flash {
    ...
    async init() {
        ...
        if (this._stlink.target_voltage < 2.0) {
            throw new Exception(
                `Supply voltage is ${this._stlink.target_voltage.toFixed(2)}V, but minimum for FLASH program or erase is 2.0V`,
            );
        }
        await this.unlock();
    }
...
class Stm32FP extends Stm32 {
    ...
    async flash_write(addr, data, { erase = false, verify = false, erase_sizes = null }) {
        ...
        this._dbg.debug(
            `Stm32FP.flash_write(${addr_str}, [data:${data.length}Bytes], erase=${erase}, verify=${verify}, erase_sizes=${erase_sizes})`,
        );
        if (addr === null) {
...
class Stm32FPXL extends Stm32FP {
    ...
    async flash_write(addr, data, { erase = false, verify = false, erase_sizes = null }) {
        ...
        let addr_str = addr !== null ? `0x${H32(addr)}` : "None";
        this._dbg.debug(
            `Stm32F1.flash_write(${addr_str}, [data:${data.length}Bytes], erase=${erase}, verify=${verify}, erase_sizes=${erase_sizes})`,
        );
        let addr_bank1, addr_bank2, data_bank1, data_bank2;
        if (addr === null) {
            addr = this.FLASH_START;
        } else {
            if (addr % 2) {
