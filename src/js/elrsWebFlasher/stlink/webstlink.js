...
export default class WebStlink {
    ...
    async find_mcus_by_flash_size() {
        ...
        this._mcus = this._mcus_by_devid.devices.filter((mcu) => mcu.flash_size == this._flash_size);
        if (this._mcus.length == 0) {
            throw new libstlink.exceptions.Exception(
                `Connected CPU with DEV_ID: 0x${H24(this._mcus_by_devid.dev_id)} and FLASH size: ${this._flash_size}KB is not supported`,
            );
        }
    }
    ...
    filter_detected_cpu(expected_cpus) {
        ...
        if (cpus.length == 0) {
            ...
            if (this._mcus.length > 1) {
                throw new libstlink.exceptions.Exception(
                    `Connected CPU is not ${expected} but detected is one of ${possibilities}`,
                );
            } else {
                throw new libstlink.exceptions.Exception(
                    `Connected CPU is not ${expected} but detected is ${possibilities}`,
                );
            }
        }
        this._mcus = cpus;
